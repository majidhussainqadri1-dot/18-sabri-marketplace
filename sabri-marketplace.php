 ...
 * Description: Direct-deal multi-vendor marketplace with seller and buyer phone/WhatsApp contact, product-linked internal chat, offers, media, voice notes, safety controls and moderation.
 * Version: 1.1.0
...
final class Sabri_Marketplace {
    ...
    public function init(): void {
        ...
        if ((string) get_option('smp_plugin_version', '') !== SMP_VERSION) {
            SMP_DB::install();
            SMP_Activator::migrate_to_direct_deal();
            SMP_Activator::ensure_marketplace_page(false);
            update_option('smp_plugin_version', SMP_VERSION);
    ...
    public function disable_marketplace_cache(): void {
        $page_id = (int) get_option('smp_marketplace_page_id');
        $is_marketplace = ($page_id && is_page($page_id)) || (int) get_query_var('smp_marketplace_app') === 1 || isset($_GET['smp-marketplace-safe']);
        if (!$is_marketplace) return;
        if (!defined('DONOTCACHEPAGE')) define('DONOTCACHEPAGE', true);
        if (!defined('DONOTCACHEOBJECT')) define('DONOTCACHEOBJECT', true);
        nocache_headers();
        header('X-LiteSpeed-Cache-Control: no-cache');
        do_action('litespeed_control_set_nocache', 'Sabri Marketplace chat and direct-deal pages are dynamic.');
    }
