...
final class SMP_Ajax {
    public static function handle(): void {
        ...
        try {
            ...
            if (defined('WP_DEBUG') && WP_DEBUG) error_log('Sabri Marketplace 1.1 error: ' . $e->getMessage());
    ...
    private static function bootstrap(): void {
        ...
        if ($uid) {
            ...
            $unread_notifications = (int) $wpdb->get_var($wpdb->prepare('SELECT COUNT(*) FROM ' . SMP_DB::table('notifications') . ' WHERE user_id=%d AND is_read=0', $uid));
    ...
    private static function product_with_seller(array $row): array {
        ...
        $p['seller'] = [
            ...
            'contactVerified' => !empty($row['contact_verified']), 'status' => (string) ($row['seller_status'] ?? ''),
    ...
    private static function store(): void {
        ...
        if (!$seller) wp_send_json_error(['message' => 'Store not found.'], 404);
    ...
    private static function seller_apply(): void {
        $uid = self::require_login(); $in = self::payload();
        foreach (['legalName','storeName','phone','whatsapp','email','country','city','address','identityType','identityNumber'] as $f) if (trim((string)($in[$f]??''))==='') wp_send_json_error(['message'=>'Please complete every required seller and contact field.'],422);
        $phone=SMP_Utils::sanitize_phone((string)$in['phone']); $wa=SMP_Utils::sanitize_phone((string)$in['whatsapp']);
        if(SMP_Utils::phone_digits($phone)===''||SMP_Utils::phone_digits($wa)==='') wp_send_json_error(['message'=>'A valid phone number and WhatsApp number are required.'],422);
        $type=sanitize_key($in['sellerType']??'individual'); if(!in_array($type,['individual','business','service','health'],true))$type='individual';
        global $wpdb; $existing=$wpdb->get_var($wpdb->prepare('SELECT id FROM '.SMP_DB::table('sellers').' WHERE user_id=%d',$uid));
        $data=[
            'user_id'=>$uid,'seller_type'=>$type,'legal_name'=>sanitize_text_field($in['legalName']),'store_name'=>sanitize_text_field($in['storeName']),
            'phone'=>$phone,'alternate_phone'=>SMP_Utils::sanitize_phone((string)($in['alternatePhone']??'')),'whatsapp'=>$wa,'email'=>sanitize_email($in['email']),
            'country'=>sanitize_text_field($in['country']),'city'=>sanitize_text_field($in['city']),'address'=>sanitize_textarea_field($in['address']),
            'identity_type'=>sanitize_text_field($in['identityType']),'identity_number'=>sanitize_text_field($in['identityNumber']),
            'business_registration'=>sanitize_text_field($in['businessRegistration']??''),'tax_number'=>sanitize_text_field($in['taxNumber']??''),'license_number'=>sanitize_text_field($in['licenseNumber']??''),
            'show_phone'=>!empty($in['showPhone'])?1:0,'show_whatsapp'=>!empty($in['showWhatsapp'])?1:0,'allow_chat'=>1,
            'allow_calls'=>!empty($in['allowCalls'])?1:0,'allow_offers'=>!empty($in['allowOffers'])?1:0,
            'preferred_contact'=>in_array(($in['preferredContact']??''),['chat','phone','whatsapp'],true)?$in['preferredContact']:'chat',
            'call_hours'=>sanitize_text_field($in['callHours']??''),'updated_at'=>SMP_Utils::now(),
        ];
        if($existing){
            $current=$wpdb->get_var($wpdb->prepare('SELECT status FROM '.SMP_DB::table('sellers').' WHERE id=%d',(int)$existing));
            if($current==='rejected')$data['status']='pending';
            $wpdb->update(SMP_DB::table('sellers'),$data,['id'=>(int)$existing]); $id=(int)$existing;
        }else{
            $data['status']=get_option('smp_require_seller_approval',1)?'pending':'approved'; $data['verification_level']='phone'; $data['contact_verified']=0; $data['created_at']=SMP_Utils::now();
            $wpdb->insert(SMP_DB::table('sellers'),$data); $id=(int)$wpdb->insert_id;
        }
        SMP_Utils::audit('seller_apply','seller',$id); wp_send_json_success(['message'=>'Seller profile saved.','seller'=>SMP_Utils::current_seller($uid)]);
    }
    ...
    private static function product_save(): void {
        $uid=self::require_login(); $seller=SMP_Utils::current_seller($uid);
        if(!$seller||($seller['status']??'')!=='approved')wp_send_json_error(['message'=>'An approved seller account is required.'],403);
        $in=self::payload(); foreach(['title','category','productType','description','price'] as $f)if(trim((string)($in[$f]??''))==='')wp_send_json_error(['message'=>'Complete all required listing fields.'],422);
        $title=sanitize_text_field($in['title']); $desc=wp_kses_post($in['description']); $matches=SMP_Utils::contains_prohibited_terms($title.' '.$desc);
        if($matches)wp_send_json_error(['message'=>'The listing contains prohibited or restricted terms: '.implode(', ',$matches)],422);
        $type=sanitize_key($in['productType']); if(!array_key_exists($type,SMP_Utils::product_types()))$type='physical';
        $category=sanitize_text_field($in['category']); $id=absint($in['productId']??0); global $wpdb;
        if($id){$owner=(int)$wpdb->get_var($wpdb->prepare('SELECT seller_id FROM '.SMP_DB::table('products').' WHERE id=%d',$id));if($owner!==(int)$seller['id'])wp_send_json_error(['message'=>'Listing not found.'],404);}
        $images=SMP_Utils::upload_product_images('images'); if(!$images&&$id){$images=SMP_Utils::decode_json($wpdb->get_var($wpdb->prepare('SELECT images FROM '.SMP_DB::table('products').' WHERE id=%d',$id))??'[]');}
        $restricted=SMP_Utils::is_restricted_category($category,$type); $status=(get_option('smp_require_product_approval',1)||$restricted)?'submitted':'published';
        $data=[
            'seller_id'=>(int)$seller['id'],'title'=>$title,'category'=>$category,'subcategory'=>sanitize_text_field($in['subcategory']??''),'product_type'=>$type,
            'condition_name'=>sanitize_key($in['condition']??'new'),'brand'=>sanitize_text_field($in['brand']??''),'sku'=>sanitize_text_field($in['sku']??''),
            'short_description'=>sanitize_textarea_field($in['shortDescription']??''),'description'=>$desc,'price'=>max(0,(float)$in['price']),'sale_price'=>max(0,(float)($in['salePrice']??0)),
            'currency'=>strtoupper(substr(sanitize_text_field($in['currency']??get_option('smp_default_currency','PKR')),0,10)),'stock_qty'=>max(0,absint($in['stockQty']??1)),
            'stock_status'=>sanitize_key($in['stockStatus']??'in_stock'),'images'=>SMP_Utils::encode_json($images),'video_url'=>esc_url_raw($in['videoUrl']??''),
            'attributes'=>SMP_Utils::encode_json(['location'=>sanitize_text_field($in['location']??''),'negotiable'=>!empty($in['negotiable'])]),
            'shipping'=>SMP_Utils::encode_json(['pickup'=>!empty($in['pickupAvailable']),'deliveryDiscussion'=>!empty($in['deliveryDiscussion'])]),
            'compliance'=>SMP_Utils::encode_json(['license'=>sanitize_text_field($in['productLicense']??''),'batch'=>sanitize_text_field($in['batchNumber']??''),'expiry'=>sanitize_text_field($in['expiryDate']??'')]),
            'allow_chat'=>1,'allow_calls'=>!empty($in['allowCalls'])?1:0,'allow_whatsapp'=>!empty($in['allowWhatsapp'])?1:0,'allow_offers'=>!empty($in['allowOffers'])?1:0,
            'pickup_available'=>!empty($in['pickupAvailable'])?1:0,'delivery_discussion'=>!empty($in['deliveryDiscussion'])?1:0,'status'=>$status,'updated_at'=>SMP_Utils::now(),
        ];
        if($id){$wpdb->update(SMP_DB::table('products'),$data,['id'=>$id]);}
        else{$data['slug']=sanitize_title($title).'-'.strtolower(wp_generate_password(7,false,false));$data['deal_status']='available';$data['created_at']=SMP_Utils::now();$data['published_at']=$status==='published'?SMP_Utils::now():null;$wpdb->insert(SMP_DB::table('products'),$data);$id=(int)$wpdb->insert_id;}
        SMP_Utils::audit('product_save','product',$id,['status'=>$status]); wp_send_json_success(['message'=>$status==='published'?'Listing published.':'Listing submitted for review.','productId'=>$id]);
    }
    ...
    private static function wishlist_toggle(): void {
        $uid=self::require_login();$id=absint(self::payload()['productId']??0);if(!$id)wp_send_json_error(['message'=>'Invalid listing.'],422);global$wpdb;
    ...
    private static function contact_reveal(): void {
        $uid=self::require_login();$in=self::payload();$product_id=absint($in['productId']??0);$seller_id=absint($in['sellerId']??0);global$wpdb;
        if($product_id){$product=$wpdb->get_row($wpdb->prepare('SELECT * FROM '.SMP_DB::table('products').' WHERE id=%d',$product_id),ARRAY_A);if(!$product)wp_send_json_error(['message'=>'Listing not found.'],404);$seller_id=(int)$product['seller_id'];}
        $seller=SMP_Utils::seller_row($seller_id);if(!$seller||$seller['status']!=='approved')wp_send_json_error(['message'=>'Seller not available.'],404);
        if(SMP_Utils::is_blocked($uid,(int)$seller['user_id']))wp_send_json_error(['message'=>'Contact is unavailable because one party has blocked the other.'],403);
        $show_phone=!empty($seller['show_phone'])&&(!$product_id||!empty($product['allow_calls']));$show_wa=!empty($seller['show_whatsapp'])&&(!$product_id||!empty($product['allow_whatsapp']));
        $phone=$show_phone?(string)$seller['phone']:'';$wa=$show_wa?(string)$seller['whatsapp']:'';
        $message=$product_id?'Hello, I am interested in your Marketplace listing: '.($product['title']??'').' '.SMP_Activator::marketplace_url(): 'Hello, I am contacting you through Marketplace.';
        SMP_Utils::audit('contact_reveal','seller',$seller_id,['productId'=>$product_id,'phone'=>$show_phone,'whatsapp'=>$show_wa]);
        wp_send_json_success(['phone'=>$phone,'alternatePhone'=>$show_phone?(string)$seller['alternate_phone']:'','whatsapp'=>$wa,'callHref'=>$phone?'tel:'.$phone:'','whatsappUrl'=>$wa?SMP_Utils::whatsapp_url($wa,$message):'','callHours'=>(string)$seller['call_hours'],'preferredContact'=>(string)$seller['preferred_contact']]);
    }

    private static function conversation_start(): void {
        $uid=self::require_login();$in=self::payload();$product_id=absint($in['productId']??0);if(!$product_id)wp_send_json_error(['message'=>'Select a listing first.'],422);
        if(get_option('smp_require_buyer_contact_before_chat',1)&&!SMP_Utils::buyer_contact_complete($uid))wp_send_json_error(['message'=>'Save your phone and WhatsApp numbers in Account before starting a chat.','needsContact'=>true],422);
        global$wpdb;$p=$wpdb->get_row($wpdb->prepare("SELECT p.*,s.user_id seller_user_id,s.status seller_status,s.allow_chat seller_allow_chat FROM ".SMP_DB::table('products')." p JOIN ".SMP_DB::table('sellers')." s ON s.id=p.seller_id WHERE p.id=%d",$product_id),ARRAY_A);
        if(!$p||$p['seller_status']!=='approved'||empty($p['allow_chat'])||empty($p['seller_allow_chat']))wp_send_json_error(['message'=>'Internal chat is unavailable for this listing.'],403);
        $seller_uid=(int)$p['seller_user_id'];if($uid===$seller_uid)wp_send_json_error(['message'=>'You cannot start a buyer chat with your own listing.'],422);if(SMP_Utils::is_blocked($uid,$seller_uid))wp_send_json_error(['message'=>'Chat is unavailable because one party has blocked the other.'],403);
        $id=(int)$wpdb->get_var($wpdb->prepare('SELECT id FROM '.SMP_DB::table('conversations').' WHERE product_id=%d AND buyer_id=%d AND seller_id=%d',$product_id,$uid,(int)$p['seller_id']));
        if(!$id){$wpdb->insert(SMP_DB::table('conversations'),['product_id'=>$product_id,'buyer_id'=>$uid,'seller_id'=>(int)$p['seller_id'],'seller_user_id'=>$seller_uid,'status'=>'active','deal_status'=>'discussing','currency'=>$p['currency']?:get_option('smp_default_currency','PKR'),'created_at'=>SMP_Utils::now(),'updated_at'=>SMP_Utils::now()]);$id=(int)$wpdb->insert_id;self::insert_system_message($id,'Conversation started about “'.$p['title'].'”. Price, payment, pickup and delivery will be arranged directly by the parties.');SMP_Utils::notify($seller_uid,'new_chat','New Marketplace inquiry',SMP_Utils::client_name($uid).' contacted you about '.$p['title'],SMP_Activator::marketplace_url().'#chats');}
    ...
    private static function message_send(): void {
        ...
        $wpdb->insert(SMP_DB::table('messages'),$data);$mid=(int)$wpdb->insert_id;$wpdb->update(SMP_DB::table('conversations'),['last_message_id'=>$mid,'last_message_at'=>SMP_Utils::now(),'updated_at'=>SMP_Utils::now()],['id'=>$cid]);
    ...
    private static function message_delete(): void {
        ...
        if(!$row)wp_send_json_error(['message'=>'Message not found.'],404);if($row['attachment_path'])SMP_Utils::delete_private_file($row['attachment_path']);
        $wpdb->update(SMP_DB::table('messages'),['message_text'=>'','attachment_path'=>'','attachment_name'=>'','attachment_mime'=>'','attachment_size'=>0,'deleted_for_all'=>1,'edited_at'=>SMP_Utils::now()],['id'=>$id]);SMP_Utils::audit('message_delete','message',$id);wp_send_json_success(['message'=>'Message deleted for both parties.']);
    }
    ...
    private static function seller_dashboard(): void {
        ...
        $rows=$wpdb->get_results($wpdb->prepare('SELECT * FROM '.SMP_DB::table('products').' WHERE seller_id=%d ORDER BY id DESC LIMIT 250',(int)$seller['id']),ARRAY_A);
    ...
    private static function notifications(): void {
        $uid=self::require_login();global$wpdb;$rows=$wpdb->get_results($wpdb->prepare('SELECT * FROM '.SMP_DB::table('notifications').' WHERE user_id=%d ORDER BY id DESC LIMIT 150',$uid),ARRAY_A);
        wp_send_json_success(['notifications'=>array_map(static fn($r)=>['id'=>(int)$r['id'],'type'=>$r['type'],'title'=>$r['title'],'body'=>$r['body'],'link'=>$r['link'],'isRead'=>!empty($r['is_read']),'createdAt'=>$r['created_at']],$rows)]);
    }

    private static function notification_read(): void {
        $uid=self::require_login();global$wpdb;$id=absint(self::payload()['notificationId']??0);$where=['user_id'=>$uid];if($id)$where['id']=$id;$wpdb->update(SMP_DB::table('notifications'),['is_read'=>1],$where);wp_send_json_success(['message'=>'Notifications marked read.']);
    }
    ...
    public static function download_chat_file(): void {
        ...
        $path=SMP_Utils::private_file_path($row['attachment_path']);if(!is_file($path)){status_header(404);exit('Not found');}
        nocache_headers();header('Content-Type: '.($row['attachment_mime']?:'application/octet-stream'));header('Content-Length: '.filesize($path));header('Content-Disposition: inline; filename="'.sanitize_file_name($row['attachment_name']?:basename($path)).'"');header('X-Content-Type-Options: nosniff');readfile($path);exit;
    }
}
